...
void xeond_pci_domain_scan_bus(struct device *dev)
{
	DEV_FUNC_ENTER(dev);
...
static void xeond_pci_dev_iterator(struct bus *bus,
		...
		void *data)
{
	...
	for (curdev = bus->children; curdev; curdev = curdev->sibling) {
		...
		for (link = curdev->link_list; link; link = link->next)
			xeond_pci_dev_iterator(link, dev_iterator, res_iterator, data);
	}
}

static void xeond_pci_dev_read_resources(struct device *dev, void *data)
{
	post_log_path(dev);
	dev->ops->read_resources(dev);
}

static void xeond_pci_dev_dummy_func(struct device *dev)
{
}

static void xeond_reset_pci_op(struct device *dev, void *data)
{
	if (dev->ops)
		dev->ops->read_resources = xeond_pci_dev_dummy_func;
}
...
static void xeond_pci_domain_read_resources(struct device *dev)
{
	struct bus *link;
	...
	for (link = dev->link_list; link; link = link->next)
		xeond_pci_dev_iterator(link, xeond_pci_dev_read_resources, NULL, NULL);

	for (link = dev->link_list; link; link = link->next)
		xeond_pci_dev_iterator(link, xeond_reset_pci_op, NULL, NULL);
...
void xeond_pci_domain_set_resources(struct device *dev)
{
	DEV_FUNC_ENTER(dev);

	print_resource_tree(dev, BIOS_SPEW, "Before xeond pci domain set resource");

	/* reset bus 0 dev resource assignment - need to change them to FSP IIOStack window */
	xeond_pci_dev_iterator(dev->link_list, NULL, reset_resource_to_unassigned, NULL);

	/* update dev resources based on IIOStack IO/Mem32/Mem64 windows */
	xeond_pci_domain_read_resources(dev);
	...
	print_resource_tree(dev, BIOS_SPEW, "After xeond pci domain set resource");
