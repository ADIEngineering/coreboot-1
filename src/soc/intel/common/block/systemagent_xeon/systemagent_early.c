...
void bootblock_systemagent_early_init(void)
{
	...
	/*
	 * The MMCFG_BASE_BAR is assumed to live in the memory mapped IO space under
	 * 4GiB.
	 */
	reg = 0;
	pci_io_write_config32(SA_DEV_ROOT, MMCFG_BASE_BAR + 4, reg);
	...
	pci_io_write_config32(SA_DEV_ROOT, MMCFG_BASE_BAR, CONFIG_MMCONF_BASE_ADDRESS);

	pci_io_write_config32(SA_DEV_ROOT, MMCFG_LIMIT_BAR + 4, 0);
	pci_io_write_config32(SA_DEV_ROOT, MMCFG_LIMIT_BAR, (CONFIG_MMCONF_BASE_ADDRESS+limit));

	/*
	 * TSEG defines the base of SMM range. BIOS determines the base.
	 * Its better to clear TSEG register early
		...
		size_t count)
{
	...
	for (i = 0; i < count; i++) {
		...
		index = fixed_set_resources[i].index;
		/* Check if PCI BAR already enabled */
		base = pci_read_config32(DMI_DEV_ROOT, index);
		...
		base = fixed_set_resources[i].base;
		if (base >> 32)
			pci_write_config32(DMI_DEV_ROOT, index + 4, base >> 32);
		pci_write_config32(DMI_DEV_ROOT, index, (base & 0xffffffff) | 1);
	}
}
...
{
	return ALIGN_UP(pci_read_config32(SA_DEV_ROOT, TSEG+4), 1*MiB);
}

size_t sa_get_tolm(void)
{
	return ALIGN_UP(pci_read_config32(SA_DEV_ROOT, TOLM), 1*MiB);
}
#if 0
